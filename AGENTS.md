...
- Business list pages should use the shared `ListPageLayout` component at `apps/web-antd/src/components/business/list-page-layout.vue` whenever practical.
- List pages must follow this structure:
  - Top filter card: filter fields on the left (wrapped in a flex container with `gap-4`), filter actions such as `筛选` (primary button) and `重置` on the right.
  - List surface below the filter card: a white bordered table/card container with a top toolbar and the list content below it.
  - The list toolbar is part of the list surface, not a floating row. Put primary list actions such as `创建`、`新建`、`导入`、`批量删除`、`批量导入`、`批量导出`、`刷新` on the left.
  - Toolbar right side: display mode switch only when the page actually supports multiple display modes, usually `列表` / `卡片`.
  - List content below the toolbar: table, card grid, pagination, and row/card actions. Do not show a redundant table/list title above the data, and do not show a total-count label unless a specific page requires it. Do not wrap the list content in an additional outer card.
  - Pagination must be placed inside `<div class="fn-list-pagination flex items-center justify-end">` and use standard Pagination props (do not use custom invalid slots like `v-slot:total` or `v-slot:pageSize`).
  - To prevent Vue 3 route transition animation crashes, the page component template must have a single root element (e.g. wrap `<ListPageLayout>` and any parallel drawer/modal elements in a plain wrapper `<div>`).
- Keep list controls aligned and dense. Avoid mixing filter actions with list actions in the same card.
- Standardized pages include:
  - 租户侧: 存储管理 (`user-ai-asset/storage/index.vue`), 数据集管理 (`user-ai-asset/dataset/index.vue`), 镜像仓库 (`user-ai-asset/image/index.vue`), 镜像任务 (`user-ai-asset/image/task.vue`), 预置模型 (`user-ai-asset/model/preset.vue`), 自定义模型 (`user-ai-asset/model/custom.vue`), 模型服务市场 (`user-ai-asset/model-market/index.vue`), API Key管理 (`user-apikey/index.vue`), 组织管理 (`user-organization/index.vue`).
  - 管理侧: 数据集管理 (`admin-ai-asset/dataset/index.vue`), 镜像管理 (`admin-ai-asset/image/index.vue`), 模型管理 (`admin-ai-asset/model/index.vue`), 模型服务市场 (`admin-ai-asset/model-market/index.vue`).
- Prefer Vben-provided page/layout components (for example `Page` through `ListPageLayout`) and project-local shared components before adding one-off page layout code.
