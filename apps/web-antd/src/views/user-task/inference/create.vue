...
import { Button, Card, Form, FormItem, Input, Space, Switch, Divider, Select, InputNumber, Segmented } from 'ant-design-vue';
    ...
    <Card>
      ...
      <div class="flex items-center justify-between border-b pb-4 mb-4">
        <div class="flex items-center gap-2 cursor-pointer" @click="goBack">
          ...
          <span class="text-lg font-semibold text-gray-800">创建推理服务 · 高级切换</span>
        </div>
        <Space>
      ...
      <Form layout="vertical" class="px-2" :model="createForm">
        ...
        <FormItem label="描述">
          <Input.TextArea :rows="4" placeholder="请输入推理服务描述" :maxlength="1024" show-count />
        </FormItem>
        ...
        <FormItem label="分布式推理">
          ...
          <div class="text-xs text-gray-400 mt-2 flex items-start">
            <Info class="mr-1 size-3 mt-0.5 shrink-0" /> 每个实例部署在多台机器上进行部署。例如需要 4 个实例，单实例部署 of 机器数为 2，则一共需要 4x2=8 台机器来部署该服务
          </div>
        </FormItem>
        ...
        <FormItem label="副本数">
          ...
          <div class="text-xs text-gray-400 mt-2 flex items-start">
            <Info class="mr-1 size-3 mt-0.5 shrink-0" /> 设置或调整部署的副本数量，可提高部署的吞吐量和响应速度
          </div>
        </FormItem>
      ...
      <div class="flex items-center justify-between mt-8 border-t pt-4">
        <Space>
          <Button type="primary" @click="notify('已提交推理任务')">确认</Button>
