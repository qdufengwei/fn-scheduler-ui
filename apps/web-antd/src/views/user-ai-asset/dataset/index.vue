...
const filteredData = computed(() => {
  ...
  if (filterKeyword.value) {
    ...
    result = result.filter((r) =>
      r.name.toLowerCase().includes(kw) || String(r.id).includes(kw)
    );
  }
...
function handleCreateSave() {
  ...
  if (createForm.value.storageType === 's3' && (!createForm.value.accessKey || !createForm.value.secretKey || !createForm.value.s3Address)) {
...
function handleVersionSave() {
  ...
  const record = dataSource.value.find((item) => item.id === selectedDataset.value.id);
  if (record) {
  ...
  <ListPageLayout>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'name'">
          <div class="flex flex-col">
            ...
            <span class="text-xs text-gray-400 dark:text-zinc-500">ID: {{ record.id }}</span>
          </div>
        </template>
        ...
        <template v-if="column.dataIndex === 'progress'">
          <div class="flex items-center gap-2">
            <Progress
              ...
              :status="record.status === '上传失败' ? 'exception' : (record.progress === 100 ? 'success' : 'active')"
              size="small"
              style="width: 100px"
              :show-info="false"
            />
        ...
        <template v-if="column.dataIndex === 'action'">
          <div class="flex items-center gap-1">
            <Button type="link" size="small" @click="handleAddVersion(record)">添加版本</Button>
            <Popconfirm
              title="确认删除该数据集吗？"
              ok-text="确认"
              cancel-text="取消"
    ...
    <CreateDrawer>
      <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }" label-align="left" :model="createForm">
        ...
        <FormItem label="数据集名称" required>
          <Input v-model:value="createForm.name" placeholder="请输入数据集名称" />
          <div class="text-xs text-gray-400 mt-1">
            ① 名称只允许输入大小写字母、-、数字和下划线
          </div>
        </FormItem>
        <FormItem label="描述">
          <Input.TextArea v-model:value="createForm.description" :rows="4" placeholder="请输入数据集描述" :maxlength="1024" show-count />
        </FormItem>

        <Divider />

        <!-- 版本信息 -->
        <h3 class="font-bold mb-6 text-base border-b pb-2 text-gray-900 dark:text-zinc-150">版本信息</h3>
        <FormItem label="版本">
          <div class="text-gray-800 dark:text-zinc-300 font-medium py-1">v1</div>
        </FormItem>
        <FormItem label="存储类型" required>
          <div class="flex gap-3">
            <button 
              ...
              :class="createForm.storageType === 'share' ? 'border-blue-500 text-blue-500 bg-blue-50/10 font-medium' : 'border-gray-200 text-gray-600 dark:text-zinc-400 dark:border-zinc-800 hover:border-gray-300'"
            ...
            <button 
              type="button" 
              class="px-5 py-2 border rounded text-sm transition-all"
              :class="createForm.storageType === 's3' ? 'border-blue-500 text-blue-500 bg-blue-50/10 font-medium' : 'border-gray-200 text-gray-600 dark:text-zinc-400 dark:border-zinc-800 hover:border-gray-300'"
        ...
        <template v-if="createForm.storageType === 'share'">
          <FormItem label="数据集地址" required>
            <Input v-model:value="createForm.path" placeholder="请输入数据集地址" />
            <div class="text-xs text-gray-400 mt-1 leading-relaxed">
              ① 地址为Linux绝对路径，必须是文件或目录，例如 /data/dataset/corpus.zip
            </div>
          </FormItem>
        </template>
        ...
        <template v-else>
          <FormItem label="Access Key" required>
            <Input v-model:value="createForm.accessKey" placeholder="请输入Access Key" />
          </FormItem>
          <FormItem label="Secret Access Key" required>
            <Input v-model:value="createForm.secretKey" placeholder="请输入Secret Access Key" type="password" />
          </FormItem>
          <FormItem label="S3地址" required>
            <Input v-model:value="createForm.s3Address" placeholder="/bucketName/subPath 或 https://s3Endpoint.region.amazonaws.com/bucketName/subPath" />
          </FormItem>
        </template>

        <FormItem label="版本描述">
          <Input.TextArea v-model:value="createForm.versionDescription" :rows="4" placeholder="请输入版本描述" :maxlength="1024" show-count />
        </FormItem>
      </Form>
      <template #footer>
    ...
    <VersionDrawer>
      <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }" label-align="left" :model="versionForm">
        ...
        <h3 class="font-bold mb-6 text-base border-b pb-2 text-gray-900 dark:text-zinc-150">基础信息</h3>
        <FormItem label="版本">
          <div class="text-gray-800 dark:text-zinc-300 font-medium py-1">{{ versionForm.version }}</div>
        </FormItem>
        ...
        <FormItem label="数据集地址" required>
          <Input v-model:value="versionForm.path" placeholder="请输入数据集绝对地址" />
        </FormItem>
        <FormItem label="版本描述">
