...
const keyword = ref('');
const rows = ref([
  { id: 'TM-001', name: 'terminal-test01-1', runtime: 'JupyterLab', status: '运行中', endpoint: 'https://terminal.demo/1', created: '2026-05-26 10:20' },
  { id: 'TM-002', name: 'terminal-test01-2', runtime: 'VSCode', status: '已停止', endpoint: '-', created: '2026-05-25 13:00' },
]);
</script>
...
<template>
  <div class="min-h-full bg-gray-50 p-4">
    <Card class="mb-4">
      <Space wrap>
        <Input v-model:value="keyword" placeholder="搜索终端名称/ID" style="width: 260px" />
    ...
    <Card title="云终端列表">
      <Table
        row-key="id"
        :data-source="rows.filter((r) => !keyword || `${r.id}${r.name}`.includes(keyword))"
        :columns="[
